...
//
// ## Двухуровневая система управления портами
//
// Пакет реализует специальную систему для предотвращения двойного освобождения портов:
//
// 1. **Основной порт (Primary Port)**:
//    - Выделяется BuilderManager'ом при вызове CreateBuilder()
//    - Используется для первого медиа потока в SDP
//    - Освобождается ТОЛЬКО BuilderManager'ом при вызове ReleaseBuilder()
//    - НЕ добавляется в список allocatedPorts внутри builder'а
//    - Builder использует его напрямую из config.LocalPort
//
// 2. **Дополнительные порты (Additional Ports)**:
//    - Выделяются из PortPool для дополнительных медиа потоков (multi-stream)
//    - Отслеживаются в списке builder.allocatedPorts
//    - Освобождаются builder'ом при вызове Close()
//    - Используются когда в SDP больше одного медиа описания
//
// Эта архитектура гарантирует:
//   - Отсутствие двойного освобождения портов
//   - Корректную работу с множественными медиа потоками
//   - Четкое разделение ответственности между Manager и Builder
//
// # Обработка ошибок
//
