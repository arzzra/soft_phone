...
func ExampleStack() {
	...
	udpTransport, err := transport.NewUDPTransport("0.0.0.0:5060")
	if err != nil {
		log.Fatal(err)
	}
	transportMgr.AddTransport(udpTransport)
...
func ExampleDialog_Refer() {
	...
	transferTarget := types.NewSipURI("bob", "example.com")
	err := dialog.Refer(context.Background(), transferTarget, dialog.ReferOpts{})
	if err != nil {
