...
func TestCallbacks(t *testing.T) {
	...
	// Тестируем OnReInvite callback
	reInviteCallbackSet := false
	ua.OnReInvite(func(d dialog.IDialog, tx dialog.IServerTX) {
		reInviteCallbackSet = true
	})
	
	// Проверяем, что можем установить callback без паники
	assert.False(t, reInviteCallbackSet, "ReInvite callback should not be called")
