...
### MediaDirection
- **DirectionSendRecv**: Двунаправленный поток (по умолчанию)
- **DirectionSendOnly**: Только отправка аудио
- **DirectionRecvOnly**: Только прием аудио  
- **DirectionInactive**: Медиа неактивно

```go
// Изменение режима во время работы
session.SetDirection(media.DirectionSendOnly)

// Проверка разрешений
canSend := session.GetDirection() == media.DirectionSendRecv || 
           session.GetDirection() == media.DirectionSendOnly
```
...
Этот код предоставлен как пример реализации медиа слоя для образовательных целей. 